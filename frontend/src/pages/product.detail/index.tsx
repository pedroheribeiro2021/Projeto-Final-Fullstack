          ...
          <div className="vehicle-apresentation">
            <div className="vehicle-informations">
              <div className="container_img">
                <img className="princialp-image" src={coverImage?.imageUrl} alt="" />
              </div>
                  ...
                  <div className="photos_gallery">
                    {
                      announcement.images.map((image:any, i:number) => {
                        return (
                          <>
                          <img onClick={() => setModalIsOpen(true)} src={image.imageUrl} alt="" />
                          <ModalImageStyled
                            isOpen={modalIsOpen}
                ...
                <ul className="container_list_coments">
                  {
                    comments.length ? (
                      comments.map((announcement) =>
                        announcement.comments.map((comment:any, i:number) => 
                        <li key={i}>
                          <div className="header-coment">
                            <img id="user" src={user1} alt="" />
                            <span className="body-2-500">{comment.user.name}</span>
                            <img src={ellipse} alt="" />
                            <span className="past_time">{moment(comment.createdAt).fromNow()}</span>
                          </div>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="fast_comments">
                      ...
                      <span onClick={()=>createComment({
                        ...
                        })}>Recomendarei para meus amigos</span>
                    </div>
                  </form>
                </div>
              </div>
